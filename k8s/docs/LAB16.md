...
- Prometheus Operator
  - Controls lifecycle management for Prometheus and Alertmanager
  - Reconciles monitoring components through CRDs
  - Watches ServiceMonitor and PodMonitor objects

- Prometheus
  - Central time-series database for metrics
  - Scrapes Kubernetes API, node-exporter, and kube-state-metrics
  - Retains data according to configured policies
  - Supports metric analysis with PromQL

- Alertmanager
  - Receives alerts generated by Prometheus
  - Performs deduplication, grouping, and routing
  - Delivers notifications to configured channels

- Grafana
  - Visualizes metrics using dashboards
  - Uses Prometheus as a primary data source
  - Provides ready-made Kubernetes dashboard templates

- kube-state-metrics
  - Exposes metrics about Kubernetes object state
  - Reports details for Deployments, Pods, Nodes, PVCs, and more

- node-exporter
  - Runs as an agent on every node
  - Collects host-level OS metrics
  - Focuses on node health rather than Kubernetes object state
